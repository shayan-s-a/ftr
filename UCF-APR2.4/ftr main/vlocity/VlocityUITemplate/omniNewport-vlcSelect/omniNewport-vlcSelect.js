vlocity.cardframework.registerModule.controller('vlcSelectController', ['$scope','$timeout','$q','ouiBaseService', function($scope, $timeout, $q, ouiBaseService) {

    $scope.UNIList = [];
    $scope.elementType = $scope.child.eleArray[0].name.includes('Spoke') ? 'SpokeId' : 'HubId';
    $scope.track = {};
    $scope.valuesToRetrieve = [
        {
            property: 'HubServiceAccountId',
            label: 'HubServiceLocationId'
        },
        {
            property: 'SpokeServiceAccountId',
            label: 'SpokeServiceLocationId'
        },
        {
            property: 'HubServiceAccountName',
            label: 'HubServiceLocationName'
        },
        {
            property: 'SpokeServiceAccountName',
            label: 'SpokeServiceLocationName'
        },
        {
            property: 'EVCLocation',
            label: 'EVCLocationDetail'
        },
        {
            property: 'rootBundle',
            label: 'RootBundle'
        ...
        return $q(function (resolve, reject) {
            ...
            .then(function(data){
                ...
                if (result.OBDRresp != null && result.OBDRresp.OrderItem != null) {
                    var uniId = result.OBDRresp.OrderItem[$scope.elementType];
    ...
    $scope.getUNIs = function () {
        ...
        if (Array.isArray($scope.bpTree.response.UNI)) {
            angular.forEach($scope.bpTree.response.UNI, function(value, key) {
                if ($scope.child.eleArray[0].name.includes('Spoke')) {
                    if (value.ServiceAccountId == $scope.SpokeServiceAccountId) {
                        $scope.UNIList.push({
                            'name':  'UNI '  + value.UNIHostName,
                            'value': value.UNIHostId
                        });
                    }
                } else if ($scope.child.eleArray[0].name.includes('Hub')) {
                    if (value.ServiceAccountId == $scope.HubServiceAccountId) {
                        $scope.UNIList.push({
                            'name':  'UNI ' + value.UNIHostName,
                            'value': value.UNIHostId
                        });
            ...
            if ($scope.child.eleArray[0].name.includes('Spoke')) {
                if ($scope.bpTree.response.UNI.ServiceAccountId == $scope.SpokeServiceAccountId) {
                    $scope.UNIList.push({
                ...
                if ($scope.bpTree.response.UNI.ServiceAccountId == $scope.HubServiceAccountId) {
                    $scope.UNIList.push({
                        'name':  'UNI '  + $scope.bpTree.response.UNI.UNIHostName,
                        'value': $scope.bpTree.response.UNI.UNIHostId
                    });
