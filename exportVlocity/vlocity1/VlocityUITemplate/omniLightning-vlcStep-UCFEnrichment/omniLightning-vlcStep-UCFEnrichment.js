vlocity.cardframework.registerModule.controller('vltStepController', ['$scope','$rootScope', 'bpService', '$timeout','$route', function ($scope, $rootScope, bpService, $timeout, $route) {
    ...
    if ($scope.child.name == 'SelectServiceLocation') {
        $scope.$watch('bpTree.response.SelectServiceLocation.userCanEdit', function(newVal,oldVal){
    ...
    $scope.goNext = function (step) {
        ...
        var countOfLocation = parseInt(getElementValue(bpPaths.countOfLocations));
        if (stepIsValid(step)){
            if (step.name == 'SelectServiceLocation' && $scope.bpTree.response.ServiceAccounts.find( ({Id, checked})=> {return Id === $scope.bpTree.response.SelectServiceLocation.CurrentLocationId && !checked})) {
                setElementValue(bpPaths.countOfLocations, calculateCountOfLocation());
                $scope.nextRepeater(step.nextIndex, step.indexInParent);
                $rootScope.loading = false;
            } else if(step.name != $scope.lastStepName) {
                $scope.nextRepeater(step.nextIndex, step.indexInParent);
                $rootScope.loading = false;
            } else if (step.name == $scope.lastStepName && !$scope.bpTree.response.Finished) {
                if (countOfLocation > 0) {
                    $scope.nextRepeater(step.nextIndex, step.indexInParent);
                    // $scope.nextRepeater($scope.getStep('SelectServiceLocation').indexInParent, step.indexInParent);
                    $scope.nextRepeater($scope.getStep('GetServiceAccounts').indexInParent, step.indexInParent);
                    $rootScope.loading = false;
                }
            } else if ($scope.bpTree.response.Finished) {
                $scope.nextRepeater(step.nextIndex, step.indexInParent);
                $rootScope.loading = false;
            }
        } else {
    ...
    function calculateCountOfLocation() {
        $scope.bpTree.response.ServiceAccounts.find( ({Id}) => Id === $scope.bpTree.response.SelectServiceLocation.CurrentLocationId).checked = true;
        return $scope.bpTree.response.ServiceAccounts.filter(({checked}) => !checked).length;
    }
    ...
    function stepIsValid (step, exit) {
        ...
        if (step.name == 'SelectServiceLocation') {
            ...
            if ($scope.bpTree.response.userCanEdit) {
                ...
                // }
                if (!$scope.bpTree.response.DIDLocationDetails.LocationMainPhoneNumber) {
                    $scope.modal.msgs.push('- Location Main Phone Number is required');
                } else if (hasValidNPANXX('DIDLocationDetails:LocationMainPhoneNumber')) {
                    $scope.modal.msgs.push('- Location Main Phone Number must match a qualified NPA/NXX.');
                }
            }
