...
export default class TimesheetRequestsAB extends LightningElement {
    @api employeeId;
    spinLoader = true;
    @api recordId;
    ...
    timesheetWiredResult;
    @wire(getTimesheetByDateByEmployee, { conId: '$employeeId', dateFromParam: '$dateFrom', dateToParam: '$dateTo', timeOption: '$timeOption', status: '$status' })
    timesheetDataWire(result) {
        this.timesheetWiredResult = result;
        if (result.data) {
            console.log(':::: timeSheetData:', result.data);
    ...
    handleUnlock() {
        ...
        unlockTimesheets({ timesheets: finalList, userId: this.recordId })
            .then(result => {
